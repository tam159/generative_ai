...
class SparkSQL:
    ...
    def _get_create_table_stmt(self, table: str) -> str:
        ...
        table_description_header = f"{table} table schema with column_name - data_type - comment:\n"
        table_description_details = "\n".join([f"{row.col_name} - {row.data_type} - {row.comment}" for row in statement])

        return table_description_header + table_description_details
